...
public class HelloServlet2 extends HttpServlet {
    ...
    private static final Logger logger = Logger.getLogger(HelloServlet2.class.getName());
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        if (pais.isBlank()) {
            errores.put("pais", "El pais es obligatorio");
        }
        if (lenguajes == null || lenguajes.length == 0) {
            errores.put("lenguajes", "Debe seleccionar almenos un lenguaje");
        }
        if (roles == null || roles.length == 0) {
            errores.put("roles", "Debe seleccionar almenos un rol");
        }
        if (idioma == null || idioma.isBlank()) {
            errores.put("idioma", "Debes seleccionar un idioma!");
        }

        if (habilitar == null || habilitar.isBlank()) {
            errores.put("habilitar", "Debes seleccionar habilitar!");
        }

        // GUARDAMOS EN BASE DE DATOS EL NUEVO USUARIO
        // Aprendera a hacerlo proximamente
        // Damos por hecho que no hay error al guardar en BD

        String message = "";
        if (errores != null && !errores.isEmpty()) {
            message = "Pedazo de error colega";
        }
        ;

        message = "Bienvenido al sistema"; // simulando que todo ha ido ok

        // message = "pedazo de error"; // si hubiera error
        //--------------------------

        // 3. GENERAR HTML DE RESPUESTA
        // Una vez que guardo el usuario devuelvo el formulario relleno con un mensaje de bienvenida

        // Añado el Map de errores en el objeto request
        // errores; es disinto de nulo si index2.jsp es llamada desde el servlet
        // errores: es nulo si index2.jsp es llamda desde el navegador
